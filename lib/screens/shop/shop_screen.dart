          ...
          child: SizedBox(
            ...
            children: [
              _MissionTile(
                ...
                title: AppStrings.t(context, 'shopDailyReward'),
                subtitle: dailyDone ? AppStrings.t(context, 'shopClaimedDaily') : AppStrings.t(context, 'shopDailyRewardDesc', {'amount': '${IapConstants.dailyLoginReward}'}),
                actionLabel: dailyDone ? null : AppStrings.t(context, 'shopClaimDaily'),
                done: dailyDone,
                onAction: dailyDone
                    ? null
                    : () async {
                        ...
                        _refreshDaily();
                        if (!claimed) {
                          AppToast.show(context, title: AppStrings.t(context, 'shopClaimedDaily'), icon: Icons.info_outline);
                        }
                      },
              ),
              _MissionTile(
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Text(
        ...
        style: AppTypography.labelBold(color: AppColors.onSurfaceVariant, size: 14),
      ),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(AppStrings.t(context, 'shopBrowse'), style: AppTypography.titleLarge()),
          const SizedBox(height: 12),
          SizedBox(
            height: 38,
                          ...
                          child: Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                const SizedBox(width: 4),
                                Text(
                                  AppStrings.t(context, isActive ? 'shopActive' : 'owned'),
...
class _ItemVisual extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (item.type == ShopItemType.skin) {
      final skin = CardStyle.get(item.id);
      if (skin.id != 'skin_default' || item.id == 'skin_default') {
        return DecoratedBox(
          decoration: BoxDecoration(
            gradient: LinearGradient(
          ...
          child: Center(child: Icon(Icons.park_rounded, color: Colors.white.withValues(alpha: 0.9), size: large ? 28 : 22)),
        );
      }
    }
